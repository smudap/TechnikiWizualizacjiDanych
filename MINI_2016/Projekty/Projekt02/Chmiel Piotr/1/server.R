...
shinyServer(function(input, output, session) {
  output$liderzyNieobecnosci = renderPlot({
    
    nieobecniClub <- nieobecni %>% filter(club==input$wybranyKlub)
    top5 <- arrange(nieobecniClub ,desc(count))
    top5 <- top5 %>%
      group_by(club) %>%
      top_n(n = 5, wt = count)
    
    p <- ggplot(top5, aes(x=reorder(surname_name, -count), y=count))+ geom_bar(stat="identity") + geom_text(aes(label=count), vjust=-0.2)
