...
ojciec <- pisa$ST17Q01
plec <- pisa$ST04Q01
levels(plec) <- c("dziewczynki", "chłopcy")
spoz <- pisa$ST08Q01
levels(spoz) <- c("0", "1-2", "3-4", "5+")
...
dane0 <- data.frame(odp11, odp12, odp13, matka, ojciec, wyk2, kraj,plec, spoz)
...
czcionki <- theme( axis.text.x = element_text( size = 2*17),
                   ...
                   axis.title.x= element_text(size = 36, vjust=-1.3, colour="grey50"),
...
polplec <- ggplot(na.omit(dane), aes(x=plec, fill=odp11)) + 
  scale_fill_manual(values=c( "PeachPuff","LightSalmon", "Coral", "Tomato", "Black")) +
  geom_bar(position="fill", colour="Black") + 
  czcionki +labs(y="",x="") +scale_y_continuous(labels=percent) +
  theme( panel.background = element_rect(colour = "white", fill="white"), 
         panel.grid.major = element_line(colour = "grey90"),
         panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank())+
  geom_hline(xintercept=0, colour="black")

polspoz <- ggplot(na.omit(dane), aes(x=spoz, fill=odp11)) + 
  scale_fill_manual(values=c( "PeachPuff","LightSalmon", "Coral", "Tomato", "Black")) +
  geom_bar(position="fill", colour="Black") + 
  czcionki +labs(y="",x="liczba spóźnień w tygodniu") +scale_y_continuous(labels=percent) +
  theme( panel.background = element_rect(colour = "white", fill="white"), 
         panel.grid.major = element_line(colour = "grey90"),
         panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank())+
  geom_hline(xintercept=0, colour="black")
...
resztaplec <- ggplot(na.omit(dane1), aes(x=plec, fill=odp11)) + 
  scale_fill_manual(values=c( "PeachPuff","LightSalmon", "Coral", "Tomato", "Black")) +
  geom_bar(position="fill", colour="Black") + 
  czcionki +labs(y="",x="") +scale_y_continuous(labels=percent) +
  theme( panel.background = element_rect(colour = "white", fill="white"), 
...
resztaspoz <- ggplot(na.omit(dane1), aes(x=spoz, fill=odp11)) + 
  scale_fill_manual(values=c( "PeachPuff","LightSalmon", "Coral", "Tomato", "Black")) +
  geom_bar(position="fill", colour="Black") + 
  czcionki +labs(y="",x="liczba spóźnień w tygodniu") +scale_y_continuous(labels=percent) +
  theme( panel.background = element_rect(colour = "white", fill="white"), 
...
wykszt <- ggplot(na.omit(dane0), aes(x=ojciec, y=matka, shape=wyk2)) + 
  geom_point(size=2*10, col="grey50") +
  ggtitle("Objaśnienie \n grupowania wykształcenia ") +
  theme( axis.text.x = element_text( size = 2*17, angle=-90, hjust=),
         axis.text.y = element_text(size = 2*17),
         axis.title.x= element_text(size = 2*20, colour="grey50"),
         axis.title.y= element_text(size = 2*20, colour="grey50"),
         ...
         panel.grid.major = element_line(colour = "grey90"),
         panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank()) +
  guides(shape = guide_legend(title="wykształcenie \n rodziców", nrow = 3)) 
...
#library("gridExtra")
grid.newpage() 
grid.text("Czy czas nauki w domu jest taki sam \n w Polsce  i w innych krajach?", vp = viewport(x=0.5, y=0.95), gp=gpar(fontsize=85,  fontface="bold"))
grid.text("Polska", vp = viewport(x=0.2, y=0.88), gp=gpar(fontsize=60, fontface="bold"))
grid.text("Reszta Świata", vp = viewport(x=0.53, y=0.88), gp=gpar(fontsize=60,  fontface="bold"))
grid.text("Nie występują znaczne różnice według płci,\n ale w Polsce generalnie uczymy się mniej", vp = viewport(x=0.36, y=0.84), gp=gpar(fontsize=40,  fontface="bold"))
grid.text("W Polsce spóźnialscy nadrabiają nauką w domu", vp = viewport(x=0.36, y=0.56), gp=gpar(fontsize=40,  fontface="bold"))
grid.text("Występuje odwrotna tendencja czasu nauki w zależności \n od wykształcenia rodziców", vp = viewport(x=0.36, y=0.28), gp=gpar(fontsize=40,  fontface="bold"))
grid.text("Autorzy: Paulina Auguścik, Barbara Sozańska, Anita Szewczyk", vp = viewport(x=0.2, y=0.01), gp=gpar(fontsize=25,  fontface="bold"))

print(polplec, vp=viewport(x=0.17, y = 0.7, 
                        width=0.33, height=0.24))
print(polspoz, vp=viewport(x=0.17, y = 0.43, 
                        width=0.33, height=0.24))
print(pol1, vp=viewport(x=0.17, y = 0.14, 
                        width=0.33, height=0.24))

print(resztaplec, vp=viewport(x=0.5, y = 0.7, 
                           width=0.33, height=0.24))
print(resztaspoz, vp=viewport(x=0.5, y = 0.43, 
                           width=0.33, height=0.24))
print(reszta1, vp=viewport(x=0.5, y = 0.14, 
                           width=0.33, height=0.24))
